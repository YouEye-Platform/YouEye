...
**VM:** potempc
**Agent:** Artem
**Task:** Remove platform overlay flicker and cold-open lag.

### Changes
- `control-panel/package.json` — bumped Control Panel to `0.4.53` before building the release artifact.
- `control-panel/src/components/control-surface/control-header.tsx` — replaced cold-mounted Settings/Market overlay iframes with persistent prewarmed drawer, launcher, and notification frames that stay hidden until loaded and post `youeye:overlay-visibility` to the UI embed.
- `control-panel/tests/control-header-drawer-icons.spec.mjs` — updated static coverage for the persistent/prewarmed overlay host contract.
- `ui/package.json` — bumped YouEye UI to `0.4.33` before building the release artifact.
- `ui/src/components/layout/platform-overlay-frame.tsx` — keeps overlay frames mounted after preload, gates pointer events/opacity on load, and sends visibility messages to embedded content.
- `ui/src/components/layout/drawer-and-launcher.tsx` — prewarms dashboard drawer/launcher overlays on idle, hover, and focus.
- `ui/src/components/layout/app-drawer.tsx` — added loading/error gates and visibility refresh handling so preloaded embeds do not show a false “No apps installed” state.
- `ui/src/components/layout/launcher.tsx` — added loading/error gates and open-time refresh handling for the launcher embed.
- `ui/src/components/layout/notification-bell.tsx` — prewarms the notification overlay and refreshes/polls the embedded notification centre only while visible.
- `ui/tests/platform-overlays-fast-open.test.mjs`, `ui/tests/notifications-e3.test.mjs` — added/updated regression coverage for fast-open overlays and the UI-owned notification shell.
- `ui/public/sw.js` — regenerated by the production build for the new UI release.
- `README.md` — updated the current-version matrix for CP, UI, Canvas, and all six native overlay-speed releases.

### Test Results
- Control Panel focused test: `pnpm --dir control-panel exec node --test tests/control-header-drawer-icons.spec.mjs` passed.
- Control Panel typecheck: `pnpm --dir control-panel run typecheck` passed.
- Control Panel build: `pnpm --dir control-panel run build` passed for `ye-controlpanel@0.4.53`.
- UI focused tests: `pnpm --dir ui exec node --test tests/platform-overlays-fast-open.test.mjs tests/launcher-e1.test.mjs tests/notifications-e3.test.mjs` passed (13/13).
- UI typecheck: `pnpm --dir ui exec tsc --noEmit --pretty false` passed.
- UI build: `pnpm --dir ui run build` passed for `ye-ui@0.4.33`.
- Diff check: `git diff --check` passed.

### Notes for Iris
- This release is paired with Canvas `v0.3.3` and the six native app releases below. The UI app list/notification data remains live: installed apps and notifications refresh on overlay-open visibility messages and existing polling/API updates.
- The account menu intentionally remains native for now per owner direction; it does not need an embed for this fix.
...
**VM:** potempc
**Agent:** Artem
**Task:** Move drawer, launcher, and notifications to UI-owned platform overlays.

### Changes
- `control-panel/package.json` — bumped Control Panel to `0.4.51` before building the release artifact.
- `control-panel/src/components/control-surface/control-header.tsx` — replaced local Settings drawer/launcher/notification popovers with UI-origin fullscreen sandboxed overlay iframes and origin-validated overlay messages.
- `ui/package.json` — bumped YouEye UI to `0.4.32` before building the release artifact.
- `ui/src/components/layout/platform-overlay-frame.tsx`, `ui/src/components/layout/embed-overlay-shell.tsx` — added shared overlay host/shell primitives for fullscreen transparent platform overlays.
- `ui/src/app/embed/drawer/page.tsx`, `ui/src/app/embed/launcher/page.tsx`, `ui/src/app/embed/notifications/page.tsx` — made the drawer, launcher, and notifications routes render full UI-owned overlay surfaces instead of content-only embeds.
- `ui/src/components/layout/drawer-and-launcher.tsx`, `ui/src/components/layout/notification-bell.tsx`, `ui/src/components/layout/app-drawer.tsx` — updated dashboard triggers and embedded content wiring for the UI-owned overlay model.
- `ui/src/app/globals.css` — made overlay embed documents transparent and overflow-hidden so hosts do not show iframe slabs or nested scrollbars.
- `ui/public/sw.js` — regenerated by the production build for the new UI release.
- `README.md` — updated the current-version matrix for CP, UI, Canvas, and all six native overlay releases.

### Test Results
- Control Panel typecheck: `pnpm --dir control-panel run typecheck` passed.
- Control Panel build: `pnpm --dir control-panel run build` passed for `ye-controlpanel@0.4.51`.
- UI typecheck: `pnpm --dir ui exec tsc --noEmit --pretty false` passed.
- UI build: `pnpm --dir ui run build` passed for `ye-ui@0.4.32`.
- Visual smoke: local Playwright screenshots verified transparent drawer, launcher, and notification overlay embeds without nested host scrollbars.

### Notes for Iris
- This release is paired with Canvas `v0.3.2` and the six native app overlay-host releases. No live update was performed from Artem's workstation because the live target was not reachable during this pass.

## ui-v0.4.31 — artem — 2026-06-22
**Branch:** main
**VM:** potempc
**Agent:** Artem
**Task:** Fix dashboard widget iframe readiness and expose identity app-data ids for Weather widgets.

### Changes
- `ui/package.json` — bumped YouEye UI to `0.4.31` before building the release artifact.
- `ui/src/components/embeds/unified-embed.tsx` — added a guarded `markReady()` path and treats widget iframe `load` as ready so rendered widgets do not falsely fall back when a child page misses `youeye:ready`.
- `ui/src/app/api/v1/header/config/route.ts` — returns `identity_id` and `app_data_user_id` alongside the existing user profile data so native apps can reconcile UI users with identity-backed app data.
- `ui/public/sw.js` — regenerated by the production build for the new UI release.
- `README.md` — updated the current-version matrix for UI and all six native app widget releases.

### Test Results
- UI typecheck: `pnpm --dir ui exec tsc --noEmit --pretty false` passed.
- UI diff check: `git diff --check` passed.
- UI build: `pnpm --dir ui build` passed for `ye-ui@0.4.31`.
- Artifact: standalone output contains root `server.js` and root `package.json` with version `0.4.31`.

### Notes for Iris
- This release is paired with the six native app widget embed releases. The owner will update and test bykapc manually.
