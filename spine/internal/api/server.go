...
func (s *Server) setupRoutes() {
	...
	s.mux.HandleFunc("/api/update/ui", s.handleUpdateUI)
...
// handleUpdateUI handles updating the UI application to the latest release.
func (s *Server) handleUpdateUI(w http.ResponseWriter, r *http.Request) {
	...
	out, err := exec.Command("incus", "list", "^"+containerName+"$", "--format", "csv", "-c", "s").Output()
	if err != nil || strings.TrimSpace(string(out)) == "" {
		errorResponse(w, "UI container not found", http.StatusNotFound)
		return
	}

	currentVersion := s.getUIVersion()
	latestVersion := s.getLatestRelease(s.cfg.Releases.Repositories.UI, s.cfg.Releases.Repositories.UITagPrefix)
	...
	if currentVersion == latestVersion {
		jsonResponse(w, map[string]string{
			"status":  "up-to-date",
			"message": fmt.Sprintf("UI is already at version %s", currentVersion),
		})
		return
	}

	update.Start("ui", currentVersion)
	update.Emit("ui", update.StatusDownloading, 10, "Creating snapshot...")

	// Create snapshot
	exec.Command("incus", "snapshot", "delete", containerName, "pre-update").Run()
	exec.Command("incus", "snapshot", "create", containerName, "pre-update").Run()

	update.Emit("ui", update.StatusDownloading, 20, fmt.Sprintf("Downloading %s...", latestVersion))

	// Download
	downloadURL, err := s.getAssetDownloadURL(s.cfg.Releases.Repositories.UI, "standalone.tar", s.cfg.Releases.Repositories.UITagPrefix)
	if err != nil {
		errorResponse(w, fmt.Sprintf("failed to get download URL: %v", err), http.StatusInternalServerError)
		return
	}
	...
	tmpFile, err := os.CreateTemp("", "ui-update-*.tar")
	if err != nil {
		errorResponse(w, "failed to create temp file", http.StatusInternalServerError)
		return
	}
	defer os.Remove(tmpFile.Name())
	...
	tmpFile.Close()
	if err != nil {
		errorResponse(w, "failed to download update", http.StatusInternalServerError)
		return
	}

	update.Emit("ui", update.StatusInstalling, 40, "Stopping UI service...")

	// Check if service was running
	wasRunning := false
	statusOut, _ := exec.Command("incus", "exec", containerName, "--",
		"systemctl", "is-active", "youeye-ui").Output()
	...
	update.Emit("ui", update.StatusInstalling, 50, "Deploying new version...")
	...
	if err := exec.Command("incus", "file", "push", tmpFile.Name(), containerName+"/tmp/update.tar").Run(); err != nil {
		exec.Command("incus", "snapshot", "restore", containerName, "pre-update").Run()
		errorResponse(w, "failed to push update", http.StatusInternalServerError)
		return
	}
	...
	if err := exec.Command("incus", "exec", containerName, "--",
		"tar", "-xf", "/tmp/update.tar", "-C", appDir, "--no-same-owner").Run(); err != nil {
		exec.Command("incus", "snapshot", "restore", containerName, "pre-update").Run()
		errorResponse(w, "failed to extract update", http.StatusInternalServerError)
		return
	}

	// Flatten: if standalone/ dir exists, move its contents up
	exec.Command("incus", "exec", containerName, "--",
		"bash", "-c", fmt.Sprintf("if [ -d %s/standalone ]; then cp -a %s/standalone/. %s/ && rm -rf %s/standalone; fi",
			appDir, appDir, appDir, appDir)).Run()

	// Copy static files if they exist
	exec.Command("incus", "exec", containerName, "--",
		"bash", "-c", fmt.Sprintf("if [ -d %s/.next/static ]; then cp -r %s/.next/static %s/.next/standalone/.next/ 2>/dev/null; fi",
			appDir, appDir, appDir)).Run()

	exec.Command("incus", "exec", containerName, "--", "rm", "-f", "/tmp/update.tar").Run()

	if wasRunning {
		update.Emit("ui", update.StatusRestarting, 70, "Starting UI service...")
		exec.Command("incus", "exec", containerName, "--", "systemctl", "start", "youeye-ui").Run()

		update.Emit("ui", update.StatusVerifying, 80, "Checking health...")
		...
		if !healthy {
			...
			update.Fail("ui", currentVersion, "health check failed, rolled back")
			errorResponse(w, "update failed, rolled back", http.StatusInternalServerError)
			return
		}
	}

	update.Complete("ui", currentVersion, latestVersion)

	jsonResponse(w, map[string]string{
		"status":      "success",
		"message":     fmt.Sprintf("UI updated from %s to %s", currentVersion, latestVersion),
		"old_version": currentVersion,
		"new_version": latestVersion,
	})
}
