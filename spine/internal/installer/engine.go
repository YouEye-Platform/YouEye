...
func findLatestSpineTag() (string, error) {
	...
	resp, err := client.Get("https://git.byka.wtf/api/v1/repos/potemsla/YouEye/releases?limit=50")
	if err != nil {
	...
	return "", fmt.Errorf("no spine release found on Gitea")
}
...
func spineDownloadURL(tag string) string {
	return fmt.Sprintf("https://git.byka.wtf/potemsla/YouEye/releases/download/%s/spine-linux-amd64", tag)
}
...
// extractIPFromAddr parses "2: eth0 inet 10.10.40.50/24 ..." style output.
