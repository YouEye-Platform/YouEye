...
if [ "$PROVIDER" = "gitea" ]; then
    ...
    # GitHub (default): API is at api.github.com, downloads at github.com
...
download_spine() {
    ...
    if curl -4 -sSL -f "$DOWNLOAD_URL" -o "$TMP_FILE" && [ -s "$TMP_FILE" ]; then
        ...
        DOWNLOAD_URL="https://raw.githubusercontent.com/${RELEASE_ORG}/${RELEASE_REPO}/main/spine/spine-linux-${ARCH}"
