...
export function AppSettingsDetail({
  ...
  const connect = async (capability: string, connectorId: string) => {
    await fetch(`/api/settings/connectors/${appId}`, {
      ...
      body: JSON.stringify({ action: "connect", capability, connectorId, persistent: true }),
    });
    fetchDetail();
  };
...
/* ── Capability Row (from connector-detail.tsx) ── */
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/settings/connectors/credentials", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ connectorId, key: field.name, value: value.trim(), boundHost: null }),
      });
      if (res.ok) {
        setSaved(true);
        setValue("");
    ...
    <div className="border rounded-lg p-4">
      <div className="flex items-center justify-between">
        ...
        {!cap.multiple && hasConnection ? (
          <button onClick={() => onDisconnect(cap.capability)} className="text-xs text-destructive hover:underline">
            {t("disconnect")}
          </button>
        ) : null}
      </div>
      ...
      {hasConnection && (
        <div className="mt-2 space-y-1.5">
          {cap.connections.map((conn) => {
            ...
            return (
              <div key={conn.id} className="flex items-center justify-between py-1.5 px-3 rounded-md bg-accent/30">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">{connector?.name ?? conn.connectorId}</span>
          ...
          {cap.connections.map((conn) => {
            ...
            const hasManagedFields = connector.configFields.some((f) => f.managed && f.required);
            const manualFields = connector.configFields.filter((f) => f.required && f.type === "secret" && !f.managed);
            ...
            return (
              <div key={`cred-${conn.id}`} className="mt-2 pl-3 border-l-2 border-amber-300">
                {hasManagedFields && connector.authProvider && !connector.authProviderConnected && (
                  <div className="mb-2">
                    <a
                      href={`/api/auth/providers/${connector.authProvider}?redirect_uri=${encodeURIComponent(typeof window !== "undefined" ? window.location.pathname : "/settings/apps")}`}
                ...
                {manualFields.length > 0 && (
                  <>
                    ...
                    {manualFields.map((field) => (
                      <CredentialEntry key={field.name} connectorId={conn.connectorId} field={field} onSaved={onRefresh} />
                    ))}
                  </>
                )}
              </div>
            ...
            <div className="space-y-1.5">
              {cap.availableConnectors
                ...
                .map((connector) => (
                  <button
                    key={connector.id}
                    onClick={() => { onConnect(cap.capability, connector.id); setShowPicker(false); }}
