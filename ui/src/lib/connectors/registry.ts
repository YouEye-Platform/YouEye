/**
 * Connector Registry — Direct Gitea Fetch
 *
 * One-Way Bridge: UI fetches connector manifests directly from YE-AppMarket
 * Gitea repo instead of proxying through CP.
 *
 * Adapted from CP's lib/connectors/registry.ts.
 */
...
/** Branch to fetch from — configurable via env, defaults to main */
...
async function fetchFile(filePath: string): Promise<string> {
  ...
  // Fallback to main if configured branch fails
  if (!res.ok && branch !== "main") {
    const fallbackUrl = rawUrl(filePath, "main");
    const fallbackRes = await fetch(fallbackUrl, {
      signal: AbortSignal.timeout(15_000),
    });
    if (!fallbackRes.ok)
      throw new Error(`Failed to fetch ${filePath}: ${fallbackRes.status}`);
...
const manifestCache = new Map<
  string,
  { manifest: ConnectorManifest; fetchedAt: number }
>();
...
export async function fetchConnectorManifest(
  ...
  if (manifest.metadata.id !== connectorId) {
    throw new Error(
      `Manifest ID "${manifest.metadata.id}" doesn't match "${connectorId}"`
    );
  }
 ...
 */
export async function findConnectorByCapability(
  capability: string
...
/**
 * Clear the connector cache. Called when catalog might have been updated.
 */
