...
export function SiteName({ name, style, className, as: Tag = "span" }: SiteNameProps) {
  ...
  const charShape = s.charShapeId
    ? CHARACTER_SHAPE_PRESETS.find((preset) => preset.id === s.charShapeId) ?? null
    : null;

  if (charShape) {
    const intensity = s.charShapeIntensity ?? 1;
    return (
      <Tag className={className} style={{ ...cssProps, display: "inline-flex", alignItems: "baseline" }}>
        {name.split("").map((ch, i) => (
          <span
            key={`${ch}-${i}`}
            style={{ display: "inline-block", transform: charShape.charTransform(i, name.length, intensity) }}
          >
            {ch === " " ? "\u00A0" : ch}
          </span>
        ))}
