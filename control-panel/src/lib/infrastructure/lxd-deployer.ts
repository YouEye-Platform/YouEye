...
async function installNodeAndApp(
  ...
  // Download and deploy app from Gitea release (branch-aware)
  const releasesURL = `${cfg.giteaBaseURL}/api/v1/repos/${cfg.giteaOrg}/${cfg.giteaRepo}/releases?limit=50`;
  ...
  const downloadScript = `
    DOWNLOAD_URL=$(curl -sSL '${releasesURL}' | \
      python3 -c "
import sys, json, re
${branchFilter}
${tagPrefixFilter}
